...
def change_permissions(path):
    ...
    for root, dir, files in os.walk(path): # root -> root directory, dir -> sub directories in the root directory, files -> the files stored in the root and sub directories
        for d in dir:
            full_dir = os.path.join(root, d)  # Get the full path to the directory
            mode = os.stat(full_dir).st_mode  # Get current permission mode
            os.chmod(full_dir, mode | stat.S_IWUSR | stat.S_IXUSR)  # Add write and execute permissions for the user
        
        for f in files:
            full_file = os.path.join(root, f)  # Get the full path to the file
            mode = os.stat(full_file).st_mode  # Get current permission mode
            os.chmod(full_file, mode | stat.S_IWUSR)  # Add write permission for the user
    
    top_mode = os.stat(path).st_mode
    flags = stat.S_IWUSR
    if os.path.isdir(path):  # check whether the path is a directory and then adds execute permission to that as well
        flags |= stat.S_IXUSR
...
def create_report(txt_files, txt_file_count, save_dir):
    ...
    report = os.path.join(save_dir, base_report_name + extension)  # Construct initial report path
    ...
    with open(report, "w") as report_file:
        ...
        report_file.write(f"Total .txt files with 8 channel data were: {txt_file_count}\n")  # Total count
