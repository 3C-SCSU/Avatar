...
def runScript():
    ...
    for dirpath, dirname, filenames in os.walk(NEW_DIRECTORY):
        for f in filenames:
            ...
            new_file_date = str(datetime.datetime.now() - datetime.timedelta(days=rand_days,seconds = rand_secs,minutes = rand_mins)) 
      
            os.system(f'sudo touch -c -d {new_file_date} {new_file_path}')

            #Log new file metadata to ouput log
            FILE.write(f"File is now at {new_file_path}, creation time changed to : {new_file_date}\n")
