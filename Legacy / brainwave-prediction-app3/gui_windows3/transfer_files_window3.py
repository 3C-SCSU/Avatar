...
class TransferFilesWindow(QWidget):
    ...
    def loadUi(self):
        ui_file_name = os.path.join(
            "./",
            "brainwave-prediction-app3",
            "gui_windows3",
            "transfer_files_window3.ui",
        )
    ...
    def save_config(self):
        selected_file, _ = QFileDialog.getSaveFileName(
            self, "Save config file", "", "INI Files (*.ini)"
        )
    ...
    def load_config(self):
        selected_file, _ = QFileDialog.getOpenFileName(
            self, "Load config file", "", "INI Files (*.ini)"
        )
        ...
        try:
            if selected_file:
                ...
                self.widget.private_key_dir_input.setText(
                    self.config["data"]["-PRIVATE_KEY-"]
                )
                self.widget.ignore_host_key_checkbox.setChecked(
                    self.config["data"]["-IGNORE_HOST_KEY-"].lower() in ("true")
                )
            ...
            QMessageBox.critical(
                None,
                "Loading failed",
                "Please ensure that your config file is not invalid or corrupted\n\nERROR:\n"
                + str(e),
            )
        ...
        try:
            ...
            if source_dir and target_dir:
                ...
                QMessageBox.critical(
                    None,
                    "Upload failed",
                    "Please ensure that all fields have been filled!",
                )
            ...
            QMessageBox.critical(
                None,
                "Upload failed",
                "Please ensure that your inputs are correct and that the server is running\n\nERROR:\n"
                + str(e),
            )
