...
def runScript():
    ...
    for dirpath, dirname, filenames in os.walk(DATASET_PATH):
        for filename in filenames:
            ...
            FILE.write(f"File {file_path} creation timestamp was {old_date['created']} before modification \n")
    ...
    shutil.copytree(DATASET_PATH,INTERMEDIATE_DATASET_PATH, ignore = shutil.ignore_patterns('*.csv'))
    ...
    for i in range(num_of_subdirs):
        ...
        for dirpath, dirname, filenames in os.walk(os.path.join(DATASET_PATH,dirnames[0][i])):
            for f in filenames:
                name,ext = os.path.splitext(f)
                
                src = os.path.join(dirpath,f)
                dst = os.path.join(INTERMEDIATE_DATASET_PATH,dirnames[0][i],f"{new_file_names.pop()}{ext}")
    ...
    for dirpath, dirname, filenames in os.walk(NEW_DIRECTORY):
        for f in filenames:
            ...
            new_file = filedate.File(os.path.join(dirpath,f))
            
            new_file.set(created=datetime.datetime.now() - datetime.timedelta(days=rand_days,seconds = rand_secs,minutes = rand_mins))

            new_date = new_file.get()
            
            
            FILE.write(f"File is now at {os.path.join(dirpath,f)}, creation time changed to : {new_date['created']}\n")
